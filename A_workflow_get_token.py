...
CHROME_EXTRA_ARGS = [
    ...
    "--disable-features=Translate,BackForwardCache,DeviceBoundSessionCredentials",
...
class TokenCollector:
    ...
    async def _ensure_request_blocking(self):
        ...
        # Shared pages skip context.route (already set by base collector)
        if not self._routes_applied and not self._shared_browser:
            try:
    ...
    def _mode_check_xpath(self, mode: str) -> str:
        mapping = {
            "video": "button[aria-haspopup='menu']:has-text('Video'), button[aria-haspopup='listbox']:has-text('Video')",
            "generate_image": "button[aria-haspopup='menu']:has-text('Nano'), button[aria-haspopup='menu']:has-text('Imagen'), button[aria-haspopup='menu']:has-text('Image'), button[aria-haspopup='menu']:has-text('Ảnh'), button[aria-haspopup='listbox']:has-text('Nano'), button[aria-haspopup='listbox']:has-text('Imagen'), button[aria-haspopup='listbox']:has-text('Image'), button[aria-haspopup='listbox']:has-text('Ảnh')",
        }
        return mapping.get(mode, "")

    def _mode_tab_xpath(self, mode: str) -> str:
        mapping = {
            "video": "button[role='tab']:has-text('Video')",
            "generate_image": "button[role='tab']:has-text('Image'), button[role='tab']:has-text('Ảnh')",
        }
        return mapping.get(mode, "")
    ...
    async def _wait_web_stable_by_create_button(self, timeout_ms: int = 30000) -> bool:
        if not self.page:
            return False
        create_btn_xpaths = [
            ".//button[i[normalize-space()='arrow_forward']]",
            ".//button[i[contains(@class, 'google-material-icons') and normalize-space()='arrow_forward']]",
            ".//button[contains(., 'Tạo') or contains(., 'Create')]",
        ]
        create_btn_css_selectors = [
        ...
        while time.time() < deadline:
            if self._should_stop():
                return False
            for xpath in create_btn_xpaths:
                btn = await self._first_visible_locator(xpath, timeout_ms=200)
                if btn is not None:
                    self._log("✅ Web Stabled")
                    return True
            for css_sel in create_btn_css_selectors:
                try:
                    loc = self.page.locator(css_sel).first
                    if await loc.is_visible():
                        self._log("✅ Web Stabled (CSS fallback)")
                        return True
                except Exception:
                    pass
            await asyncio.sleep(0.3)
        self._log("⚠️ Quá thời gian waiting web stable: chưa thấy nút Tạo/Create")
        return False
    ...
    async def _is_mode_menu_open(self) -> bool:
        if not self.page:
            return False
        try:
            video_tab = self._get_locator(self._mode_tab_xpath('video'))
            image_tab = self._get_locator(self._mode_tab_xpath('generate_image'))
    ...
    async def _switch_to_video_mode(self, skip_initial_verify: bool = False):
        ...
        if self._pool_mode:
            self._log("⚠️ Chuyển mode video thất bại, sẽ tự restart Chrome")
            return False
    ...
    async def _close_browser(self):
        # ✅ Shared page mode: chỉ đóng page riêng, không đóng browser/context
        if self._shared_browser:
            ...
            try:
                ...
                pass
            self.page = None
            # Giữ nguyên browser, context, playwright (thuộc về base collector)
            return
    ...
    async def close_after_workflow(self):
        ...
        try:
            ...
            pass
        if not self._pool_mode and not self._shared_browser:
            try:
                ChromeProcessManager.close_chrome_gracefully()
            except Exception:
                pass
    ...
    async def restart_browser(self):
        ...
        # ✅ Shared page mode: chỉ tạo lại page mới, không restart Chrome
        if self._shared_browser:
            async with self._restart_lock:
                self._log("🔄 Tạo lại page (shared browser mode)...")
                try:
                    if self.page and not self.page.is_closed():
                        self.page.off("response", self._on_response)
                        await self.page.close()
                ...
                try:
                    if self.context:
                        ...
                        self.page.on("response", self._on_response)
                        await self._apply_request_blocking()
                        self._log("✅ Đã tạo lại page thành công")
                        return True
                    else:
                        self._log("⚠️ Context không tồn tại, không thể tạo page")
                        return False
                except Exception as e:
                    self._log(f"⚠️ Lỗi tạo lại page: {e}")
                    return False
    ...
    async def _trigger_token_once(self, clear_storage=False, _has_reloaded=False):
        ...
        web_stable = await self._wait_web_stable_by_create_button(timeout_ms=15000)
        if not web_stable:
            max_reload_attempts = 3
            for reload_attempt in range(1, max_reload_attempts + 1):
                self._log(f"🔄 Web chưa ổn định, reload trang (lần {reload_attempt}/{max_reload_attempts})...")
                try:
                    await self.page.goto(
                        self.project_url,
                        wait_until="domcontentloaded",
        ...
        btn_selectors = [
            ...
            ("xpath", ".//button[i[contains(@class, 'google-material-icons') and normalize-space()='arrow_forward']]"),
            ("xpath", ".//button[contains(., 'Tạo') or contains(., 'Create')]"),
        ]
        ...
        for btn_attempt in range(max_button_retries):
            ...
            while time.time() - timeout_start < 8:
                if self._should_stop():
                    return False
                for sel_type, sel_value in btn_selectors:
                    try:
                        if sel_type == "xpath":
                            loc = await self._first_visible_locator(sel_value, timeout_ms=300)
                        else:
                            loc = self.page.locator(sel_value).last
                            try:
    ...
    async def _ensure_account_logged_in(self):
        for attempt in range(2):
            ...
            try:
                ...
                pass
            # Thử tìm cả nút tiếng Anh "New project"
            try:
                project_btn_en = self.page.locator("button:has-text('New project')").first
                if await project_btn_en.is_visible():
                    return
            except Exception:
                pass
            if attempt == 0:
        ...
        self._log(f"❌ {message}")
        self._login_required = True
        # Trong pool mode, không hiện popup, chỉ log và raise
        if not self._pool_mode:
