...
from qt_ui.i18n import t, on_lang_change
...
class StatusPanel(QWidget):
    ...
    def __init__(self, config, parent: QWidget | None = None):
        ...
        self.btn_open_guide = QPushButton(t("btn_guide"))
        ...
        self.table.setHorizontalHeaderLabels([
            t("tbl_select"), t("tbl_no"), t("tbl_video"), 
            t("lbl_status"), t("lbl_mode"), t("lbl_prompt")
        ])
        ...
        self._run_log.setStyleSheet(" border:1px solid #333;")
        ...
        self._account_group.setStyleSheet(
            "QGroupBox{font-weight:800; border:1px solid #c8d7f2; border-radius:10px; margin-top:6px;}"
        ...
        self.lb_account = QLabel(t("lbl_account"))
        self.lb_account.setStyleSheet("font-weight:800;")
        self._account_value = QLabel("Default")
        self._account_value.setStyleSheet("")

        self.lb_type = QLabel(t("lbl_account_type"))
        self.lb_type.setStyleSheet("font-weight:800;")
        self._account_type_value = QLabel("VIP1")
        self._account_type_value.setStyleSheet("")

        self.lb_expiry = QLabel(t("lbl_expiry"))
        self.lb_expiry.setStyleSheet("font-weight:800;")
        self._account_expiry_value = QLabel("-")
        self._account_expiry_value.setStyleSheet("")

        account_layout.addWidget(self.lb_account)
        account_layout.addWidget(self._account_value)
        account_layout.addSpacing(8)
        account_layout.addWidget(self.lb_type)
        account_layout.addWidget(self._account_type_value)
        account_layout.addSpacing(8)
        account_layout.addWidget(self.lb_expiry)
        ...
        on_lang_change(self._retranslate_ui)
        self._retranslate_ui()

    def _retranslate_ui(self) -> None:
        self.btn_join_video.setText(t("btn_join_video"))
        ...
        self._log_group.setTitle(t("log_title"))
        self._account_group.setTitle(t("lbl_account_info"))
        self.lb_account.setText(t("lbl_account"))
        self.lb_type.setText(t("lbl_account_type"))
        self.lb_expiry.setText(t("lbl_expiry"))
        
        # update headers
        self.table.horizontalHeaderItem(1) and self.table.horizontalHeaderItem(1).setText(t("tbl_no"))
        self.table.horizontalHeaderItem(2) and self.table.horizontalHeaderItem(2).setText(t("tbl_video"))
        self.table.horizontalHeaderItem(3) and self.table.horizontalHeaderItem(3).setText(t("lbl_status"))
        self.table.horizontalHeaderItem(4) and self.table.horizontalHeaderItem(4).setText(t("lbl_mode"))
        self.table.horizontalHeaderItem(5) and self.table.horizontalHeaderItem(5).setText(t("lbl_prompt"))
    ...
    def _apply_checkbox_button_state(self, btn: QToolButton, checked: bool) -> None:
        ...
        if checked:
            ...
            btn.setStyleSheet(
                "QToolButton {border: 1px solid #16a34a; border-radius: 3px; background: transparent; font-weight: 800;}"
            )
    ...
    def _setup_video_cell(self, row: int) -> None:
        ...
        vlabel.setStyleSheet(" font-weight:700;")
    ...
    def _refresh_video_badges(self, row: int) -> None:
        ...
        for i, btn in enumerate(buttons, start=1):
            ...
            if should_show and has_video and i == selected_idx:
                btn.setStyleSheet("border-radius:10px; font-size:10px; padding:0px; background:#2563eb;")
            elif should_show and has_video:
                btn.setStyleSheet("border-radius:10px; font-size:10px; padding:0px; background:#e5e7eb;")
            elif should_show:
                btn.setStyleSheet("border-radius:10px; font-size:10px; padding:0px;")
            try:
                btn.raise_()
            except Exception:
                pass
    ...
    def _update_status_summary(self) -> None:
        ...
        if self.lbl_status_summary is not None:
            self.lbl_status_summary.setText(
                "<span style=''>Hoàn thành(" + str(done_count) + ")</span>"
                "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
                "<span style=''>Đang tạo(" + str(creating_count) + ")</span>"
                "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
                "<span style=''>Đang chờ(" + str(waiting_count) + ")</span>"
                "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
                "<span style=''>Lỗi(" + str(failed_count) + ")</span>"
            )
    ...
    def _start_text_to_video_rows(self, rows: list[int]) -> bool:
        ...
        self._save_workflow_prompt_row_map(rows)
    ...
    def _save_workflow_prompt_row_map(self, rows: list[int]) -> None:
        """Lưu mapping prompt_id → row index để _on_video_updated luôn tìm đúng row."""
        self._workflow_prompt_row_map = {}
        for r in rows:
            pid = self._prompt_id_of_row(int(r)) or str(int(r) + 1)
            if pid:
                self._workflow_prompt_row_map[pid] = int(r)
    ...
    def _start_image_to_video_rows(self, rows: list[int], normalized_mode: str) -> bool:
        ...
        # ✅ Đồng bộ prompt IDs trước khi tạo workflow để tránh mismatch
        self._sync_stt_and_prompt_ids()
        ...
        # ✅ Lưu mapping prompt_id → row để _on_video_updated luôn tìm đúng row
        self._save_workflow_prompt_row_map(rows)
    ...
    def _on_workflow_log(self, message: str) -> None:
        ...
        # ✅ Chỉ đánh lỗi TẤT CẢ rows khi KHÔNG có ảnh nào upload thành công
        if ("upload ảnh nhân vật thất bại: tất cả" in lower_text) or ("không đọc được ảnh" in lower_text and "tất cả" in lower_text):
            self._append_run_log(message)
        ...
        row = self._find_row_by_prompt_id(prompt_id) if prompt_id else -1
        if row < 0 and prompt_id:
            # Fallback 0: dùng mapping đã lưu từ khi workflow bắt đầu
            mapping = getattr(self, '_workflow_prompt_row_map', {})
            if prompt_id in mapping:
                candidate = mapping[prompt_id]
                if 0 <= candidate < self.table.rowCount():
                    row = candidate
        if row < 0 and prompt_id:
            # Fallback 1: try matching prompt_id with STT column text
            try:
                needle = str(prompt_id or "").strip()
                for r in range(self.table.rowCount() - 1, -1, -1):
                    stt_item = self.table.item(r, self.COL_STT)
                    if stt_item is not None:
                        stt_text = str(stt_item.text() or "").strip()
                        if stt_text == needle or stt_text.lstrip("0") == needle:
                            row = r
                            break
            except Exception:
                pass
        if row < 0 and prompt_id:
            # Fallback 2: prompt_id is numeric → try as 1-based row index
            try:
                numeric_id = int(prompt_id)
                candidate_row = numeric_id - 1
                if 0 <= candidate_row < self.table.rowCount():
                    row = candidate_row
    ...
    def _on_video_updated(self, payload: dict) -> None:
        ...
        status_from_payload = str(payload.get("status") or "").strip()
        self._append_run_log(
            f"🔍 [DEBUG] _on_video_updated: prompt_id={prompt_id}, status={status_from_payload}, "
            f"table_rows={self.table.rowCount()}, "
            f"mapping={getattr(self, '_workflow_prompt_row_map', {})}"
        )
        
        row = self._find_row_by_prompt_id(prompt_id)
        self._append_run_log(f"🔍 [DEBUG] _find_row_by_prompt_id('{prompt_id}') = {row}")
        
        if row < 0:
            # Fallback 0 (ưu tiên cao nhất): dùng mapping đã lưu từ khi workflow bắt đầu
            mapping = getattr(self, '_workflow_prompt_row_map', {})
            if prompt_id in mapping:
                candidate = mapping[prompt_id]
                if 0 <= candidate < self.table.rowCount():
                    row = candidate
                    self._append_run_log(f"🔍 [DEBUG] Fallback 0 (mapping): row={row}")
        if row < 0:
            # Fallback 1: try matching prompt_id with STT column text (e.g. "005" matches prompt_id "5")
            try:
                needle = str(prompt_id or "").strip()
                for r in range(self.table.rowCount() - 1, -1, -1):
                    stt_item = self.table.item(r, self.COL_STT)
                    if stt_item is not None:
                        ...
                        if stt_text == needle or stt_text.lstrip("0") == needle:
                            row = r
                            self._append_run_log(f"🔍 [DEBUG] Fallback 1 (STT): row={row}")
                            break
            except Exception:
                pass
        if row < 0:
            ...
            try:
                ...
                if 0 <= candidate_row < self.table.rowCount():
                    row = candidate_row
                    self._append_run_log(f"🔍 [DEBUG] Fallback 2 (numeric): row={row}")
            except (ValueError, TypeError):
                pass
        if row < 0:
            self._append_run_log(f"🔍 [DEBUG] ❌ Không tìm thấy row cho prompt_id={prompt_id}, BỎ QUA update!")
            return
        ...
        if status_code == "FAILED":
            ...
            self._set_row_status_detail(row, "PENDING", self._status_text("PENDING"))
    ...
    def _load_merge_video_module(self):
        import merge_video_tool
        return merge_video_tool
