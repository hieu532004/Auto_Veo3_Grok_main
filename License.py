...
# ===== CONFIG =====
URL = "https://docs.google.com/spreadsheets/d/1u1mAifQ6STdkN2s8QjKVOKezK3Io9hBBWtzQxBLhtxQ/edit?usp=sharing"
...
def _check_license(license_key: str):
    ...
    # Chuyển đổi định dạng URL của Google Sheet sang link tải CSV
    csv_url = URL
    if "export?format=csv" not in csv_url:
        import re
        match = re.search(r"\/d\/([a-zA-Z0-9-_]+)", csv_url)
        if match:
    ...
    try:
        ...
        if status_code == 200:
            ...
            data_rows = rows[1:] if len(rows) > 0 and "License Key" in str(rows[0]) else rows
            
            for row in data_rows:
                if not row or len(row) < 1: continue
                db_key = str(row[0]).strip()
                
                if db_key == license_key:
                    db_machine_id = str(row[1]).strip() if len(row) > 1 else ""
                    expiration = str(row[2]).strip() if len(row) > 2 else ""
                    owner_name = str(row[3]).strip() if len(row) > 3 else ""
                    owner_phone = str(row[4]).strip() if len(row) > 4 else ""
                    status = str(row[5]).strip().lower() if len(row) > 5 else ""
                    
                    if status != "active":
                        return 200, {"ok": False, "ACTIVE": False, "error": "Key chưa kích hoạt hoặc đã bị khóa"}, elapsed
                        
                    if not db_machine_id:
                        return 200, {"ok": False, "ACTIVE": False, "error": f"Key chưa có Machine ID. Vui lòng dán mã máy sau vào cột B trên file Google Sheet:\n{machine_id}"}, elapsed
                    elif db_machine_id != machine_id:
                        return 200, {"ok": False, "ACTIVE": False, "error": "Key đã được sử dụng trên máy tính khác"}, elapsed
                    ...
                    if expiration:
                        try:
                            ...
                            dt = datetime.datetime.strptime(expiration.split(" ")[0].split("T")[0], "%Y-%m-%d")
                            expires_at = int(dt.timestamp())
                            if expires_at < server_ts:
                                return 200, {"ok": False, "ACTIVE": False, "error": "Key đã hết hạn"}, elapsed
                        except:
                            pass
                    ...
                    features = json.dumps({
                        "name": owner_name, 
                        "sdt": owner_phone,
                        "account": owner_name if owner_name else license_key,
                        "account_type": "VIP" if expiration else "Vĩnh viễn"
                    })
                    return 200, {
                        "ok": True, 
                        "ACTIVE": True, 
                        ...
                        "features": features,
                        "account": owner_name if owner_name else license_key,
                        "type": "VIP" if expiration else "Vĩnh viễn"
                    }, elapsed
                    
            return 200, {"ok": False, "ACTIVE": False, "error": "Key không tồn tại trên hệ thống"}, elapsed
        else:
            return status_code, {"ok": False, "ACTIVE": False, "error": f"Lỗi truy cập Google Sheet: {status_code}"}, 0.0
    except requests.exceptions.Timeout:
        return 0, {"ok": False, "ACTIVE": False, "error": "Hết thời gian chờ phản hồi"}, 0.0
    except requests.exceptions.RequestException as e:
        return 0, {"ok": False, "ACTIVE": False, "error": "Không thể kết nối internet"}, 0.0
