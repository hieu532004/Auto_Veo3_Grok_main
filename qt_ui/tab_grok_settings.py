...
from qt_ui.i18n import t, on_lang_change, TRANSLATIONS
from qt_ui.theme_manager import current_theme
...
class GrokSettingsTab(QWidget):
    ...
    def _setup_ui(self):
        # Create or clear existing layout
        if self.layout():
        ...
        self._box = QGroupBox('Cài đặt GROK')
        self._box.setStyleSheet('QGroupBox{font-weight:800;}')
        body = QVBoxLayout(self._box)
        body.setContentsMargins(10, 10, 10, 10)
        body.setSpacing(10)
        
        self._setup_box = QGroupBox('Thiết lập video GROK')
        self._setup_box.setStyleSheet('QGroupBox{font-weight:700;}')
        setup_layout = QGridLayout(self._setup_box)
        ...
        def _new_combo() -> QComboBox:
            ...
            c.setFixedHeight(30)
            c.setFixedWidth(150)
            c.setObjectName("BottomCfgCombo")
        ...
        self.lb_account_type = QLabel('Loại tài khoản:')
        self.lb_account_type.setStyleSheet('font-weight:700;')
        self.lb_length = QLabel('Thời gian video:')
        self.lb_length.setStyleSheet('font-weight:700;')
        self.lb_resolution = QLabel('Chất lượng video:')
        self.lb_resolution.setStyleSheet('font-weight:700;')
        self.lb_multi = QLabel('MULTI VIDEO:')
        self.lb_multi.setStyleSheet('font-weight:700;')
        
        try:
            self.lb_account_type.setAlignment(Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignRight)
            self.lb_length.setAlignment(Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignRight)
            self.lb_resolution.setAlignment(Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignRight)
            self.lb_multi.setAlignment(Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignRight)
            
            self.lb_account_type.setFixedWidth(115)
            self.lb_length.setFixedWidth(115)
            self.lb_resolution.setFixedWidth(115)
            self.lb_multi.setFixedWidth(115)
            
            setup_layout.addWidget(self.lb_account_type, 0, 0)
            setup_layout.addWidget(self.grok_account_type, 0, 1)
            setup_layout.addWidget(self.lb_multi, 0, 2)
            setup_layout.addWidget(self.grok_multi_video, 0, 3)
            
            setup_layout.addWidget(self.lb_length, 1, 0)
            setup_layout.addWidget(self.grok_video_length, 1, 1)
            setup_layout.addWidget(self.lb_resolution, 1, 2)
        ...
        body.addWidget(self._setup_box)
        ...
        self._note.setObjectName("HelpBoxItem")
        self._note.setStyleSheet('font-size:12px; line-height:1.45; border:1px solid #c8d7f2; border-radius:8px; padding:10px;')
    ...
    def _retranslate_ui(self):
        self._box.setTitle(t('tab_grok_settings'))
        self._setup_box.setTitle(t('settings_group_video'))
        self.lb_account_type.setText(t('lbl_account_type'))
        self.lb_length.setText(t('grok_video_length'))
        self.lb_resolution.setText(t('grok_video_resolution'))
        self.lb_multi.setText(t('grok_multi_video'))
        self.btn_open_login.setText(t('btn_open_profile'))
        self.btn_delete_profile.setText(t('btn_delete_profile'))
        self.btn_save.setText(t('btn_save'))
        
        self._note.setText(
            f"<b>{t('btn_guide') if 'btn_guide' in TRANSLATIONS else 'Hướng Dẫn'}:</b><br/>"
            "Bước 1: Bấm nút <b>Mở Chrome để đăng nhập GROK</b>, sau đó nhập tài khoản và mật khẩu để đăng nhập GROK trực tiếp trên web.<br/>"
            "Bước 2: Chọn <b>Loại Tài khoản</b> đã đăng nhập. Tài khoản thường chọn <b>NORMAL</b>, tài khoản SUPER chọn <b>SUPER</b> (chọn sai sẽ không chạy được).<br/>"
        ...
        # Correct label alignments based on newly set text
        for lb in [self.lb_account_type, self.lb_length, self.lb_resolution, self.lb_multi]:
             lb.setAlignment(Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignRight)
             lb.setFixedWidth(125 if current_lang() == 'en' else 115)
    ...
    def _show_profile_popup(self, profile_dir: str, host: str, port: int) -> None:
        ...
        status.setStyleSheet(' font-size:12px; border:1px solid #c8d7f2; border-radius:8px; padding:8px;')
