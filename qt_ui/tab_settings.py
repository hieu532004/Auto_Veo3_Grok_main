...
class SettingsTab(QWidget):
    ...
    def __init__(self, config, parent: QWidget | None = None):
        ...
        self.btn_setup_pool = QPushButton('🔧 Setup Pool Chrome')
        self.btn_setup_pool.setObjectName('Success')
        self.btn_setup_pool.setFixedHeight(36)
        self.btn_setup_pool.setToolTip('Mở từng Chrome Pool để đăng nhập Google (chỉ cần 1 lần)')
        self.btn_setup_pool.clicked.connect(self._setup_pool_chrome)
        pool_row.addWidget(self.btn_setup_pool)
    ...
    def _show_auto_login_popup(self, profile_name: str) -> None:
        ...
        log_view.setStyleSheet(' border:1px solid #333;')
    ...
    def _setup_pool_chrome(self) -> None:
        try:
            ...
            if num_chrome <= 1:
                QMessageBox.information(self, 'Thông báo', 'NUM_CHROME = 1, không cần setup pool.\nTăng NUM_CHROME > 1 trong Settings rồi Lưu trước.')
                return

            profile_name = self._current_profile_name()
            base_dir = str(sm.CHROME_USER_DATA_ROOT / profile_name)
            ...
            if not chrome_exe:
                QMessageBox.warning(self, 'Lỗi', 'Không tìm thấy Chrome. Cài Chrome trước.')
                return

            msg = f'Sẽ mở {num_chrome - 1} Chrome Pool (Chrome-1 đến Chrome-{num_chrome - 1}).\n\n'
            msg += 'Mỗi Chrome cần đăng nhập Google 1 lần.\n'
            msg += 'Sau khi đăng nhập xong, đóng Chrome và bấm OK.\n\n'
            msg += 'Bấm Yes để bắt đầu?'

            if QMessageBox.question(self, 'Setup Pool Chrome', msg,
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No) != QMessageBox.StandardButton.Yes:
                return

            for idx in range(1, num_chrome):
                pool_dir = TokenPool.get_pool_profile_dir(base_dir, idx)
                port = 9222 + idx

                cmd = [
                    chrome_exe,
                    f'--user-data-dir={pool_dir}',
                ...
                QMessageBox.information(
                    self,
                    f'Chrome Pool {idx}',
                    f'Chrome-{idx} đã mở (port {port}).\n\n'
                    f'1. Đăng nhập Google\n'
                    f'2. Tạo 1 dự án mới trên Flow\n'
                    f'3. Xong thì bấm OK\n\n'
                    f'Profile: {Path(pool_dir).name}'
                )

            QMessageBox.information(
                self,
                'Setup hoàn tất',
                f'Đã setup {num_chrome - 1} Chrome Pool.\n'
                f'Giờ đóng hết Chrome pool và chạy workflow.\n'
                f'Các Chrome sẽ tự mở khi chạy workflow.'
            )

        except Exception as exc:
            QMessageBox.critical(self, 'Lỗi', f'Lỗi setup pool: {exc}')
