"""
Grok Image-to-Video Workflow — 1:1 port of AutoGrok's I2V flow.

Flow:
  1. Open Chrome, navigate to grok.com
  2. capture_session_from_page() → GrokSession (headers + cookies)
  3. Close browser
  4. Run batch generation via direct HTTP `run_batch_image_to_video` from the API.

This module provides the synchronous entry point `run_image_to_video_jobs` for the QThread worker.
"""
...
from grok_api_image_to_video import ImageToVideoConfig, run_batch_image_to_video
from grok_api_text_to_video import GrokSession, capture_session_from_page
...
async def _run_jobs_async_ui(
    ...
    """Run image-to-video using direct HTTP batch API."""
    ...
    try:
        ...
        # We can close the browser context now because we do everything via direct HTTP.
        # This accurately mirrors AutoGrok's pattern of only using browser for auth.
        await session.close()
        ...
        # Build custom progress wrapper that respects stop_event
        ...
        # Run batch via HTTP
        results = await run_batch_image_to_video(
            items=items,
...
def run_image_to_video_jobs(
    ...
    """Synchronous entry point for QThread to run batch image-to-video jobs."""
    global DOWNLOAD_DIR
    ...
    runtime_resolution_name = str(resolution_name or "480p").strip().lower()
