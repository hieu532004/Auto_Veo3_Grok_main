...
class ImageToVideoWorkflow(QThread):
	...
	async def _process_i2v_prompt(
		...
		"""Xử lý 1 prompt image-to-video song song."""
		...
		while True:
			...
			if upload_task is None:
				...
				self._mark_prompt_failed(prompt_id, prompt_text, fail_scene_ids, "UPLOAD", "Upload task missing")
				return
			...
			if not upload_result.get("ok"):
				...
				fail_scene_ids = [str(uuid.uuid4()) for _ in range(output_count)]
				self._mark_prompt_failed(prompt_id, prompt_text, fail_scene_ids, "UPLOAD", message)
				return
			...
			if (not response.get("ok", True) or error_code_str) and (error_code_str in retryable_errors or is_auth_error):
				...
				if retry_count >= max_total_retries:
					...
					for idx_s, sid in enumerate(scene_ids):
						self._update_state_entry(prompt_id, prompt_text, sid, idx_s, "FAILED", error=error_code, message=f"Max retries exceeded")
						self.video_updated.emit({"prompt_idx": f"{prompt_id}_{idx_s + 1}", "status": "FAILED", "scene_id": sid, "prompt": prompt_text, "_prompt_id": prompt_id, "error_code": error_code_str, "error_message": f"Max retries exceeded"})
					return
			...
			if not operations and error_message:
				...
				for idx, sid in enumerate(scene_ids):
					...
					self.video_updated.emit({"prompt_idx": f"{prompt_id}_{idx + 1}", "status": "FAILED", "scene_id": sid, "prompt": prompt_text, "_prompt_id": prompt_id, "error_code": error_code_str, "error_message": error_message})
				self._discard_scene_ids(prompt_id, scene_ids)
				return
	...
	async def _status_poll_loop(self, access_token, session_id, cookie=None, auth_dict=None):
		...
		try:
			while not self.STOP:
				...
				# ✅ Ưu tiên check status qua browser (giống Text to Video) - browser có cookies session tự động
				response = None
				collector_ref = getattr(self, '_collector_ref', None)
				if collector_ref:
					...
					if browser_page and not browser_page.is_closed():
						try:
							from API_text_to_video import request_check_status_via_browser
							response = await request_check_status_via_browser(
				...
				if response is None:
					try:
						...
						self._log(
							f"⚠️ Lỗi check status (lần {self._status_poll_fail_streak}/8): {exc}"
						)
						await asyncio.sleep(5)
						continue
				...
				if not response.get("ok", True):
					...
					# ✅ Nếu 401 (token expired), xử lý giống Character Sync
					if status_code == 401:
						...
						try:
							...
							# 1. Check auth_dict - nhưng chỉ dùng nếu token KHÁC token đã lỗi  
							if auth_dict:
							...
							# 2. Dùng auth_helper force_refresh để lấy token hoàn toàn MỚI
							import auth_helper
							...
							# 3. Ép Chrome Browser reload trang + lấy token mới
							collector_ref = getattr(self, '_collector_ref', None)
