...
def _load_account_context():
    ...
    user_paygate_tier = account.get("userPaygateTier") or "PAYGATE_TIER_TWO"
    ...
    for i in range(count):
        ...
        if i > 0:
            if seed is not None:
                ...
                req["seed"] = (base_seed + i) % 100000 # Use small modulo consistent with A_workflow_sync_chactacter
            elif str(SettingsManager.load_config().get("SEED_MODE", "Random")) == "Fixed":
                # Config is Fixed: offset it
                req["seed"] = (base_seed + i) % 100000
            else:
                # Config is Random: new random seed
                req["seed"] = random.randint(0, 99999)
        else:
            req["seed"] = base_seed
...
def _send_request_with_token(url, payload, token, method="POST", cookie=None):
    # Retrieve updated token if expired
    try:
        import auth_helper
        project_id = ""
