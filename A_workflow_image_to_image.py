...
class GenerateImageWorkflow(QThread):
	...
	def _update_state_entry_unlocked(self, prompt_id, prompt_text, scene_id, idx, status, image_url="", image_path="", error="", message=""):
		state_data = self._load_state_json()
	...
	def _read_image_bytes(self, image_link):
		...
		try:
			...
			self._log(f"⚠️ Không thể xử lý ảnh: {exc}")
			return b"", ""
	...
	async def _init_token_collector(self, project_link, chrome_userdata_root, profile_name, clear_data_interval, idle_timeout, token_timeout):
		...
		self._log(f"🧭 Khởi tạo TokenCollector cho Image to Image")
	...
	async def _run_workflow(self):
		...
		async with collector:
			...
			tasks = []
			prompt_delay = max(0.1, wait_between_effective)  # Delay giữa các luồng để tránh kẹt rate limit
		...
		for retry_count in range(retry_with_error):
			try:
				...
				if page_ref and not page_ref.is_closed():
					...
					send_task = asyncio.create_task(request_generate_images(payload, access_token, cookie=cookie, project_id=effective_project_id))
				...
				if not medias:
					...
					self._log(f"❌ API không trả về ảnh (prompt {prompt_id})")
					for idx, scene_id in enumerate(scene_ids or []):
